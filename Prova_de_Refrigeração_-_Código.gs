function verificarAtencaoEnviarEmail() {
  ...
  var lastRowValues = aba.getRange(lastRow, 1, 1, 24).getValues()[0]; // Considerando até a coluna X (24 colunas)
  ...
  var tempoGasto = 'Tempo Gasto: ' + formatarTempoGasto(lastRowValues[23]); // Tempo gasto da coluna X
  ...
  // Pontuação (valor real) da coluna W
  var pontuacao = lastRowValues[22];

  // Verificação e adição de "Reprovado" ou "Aprovado"
  var resultadoProva = pontuacao < 6 ? 'Reprovado ❌' : 'Aprovado ✅';
  var resultadoProvaCor = pontuacao < 6 ? 'red' : 'green'; // Cor vermelha para reprovado, verde para aprovado
  ...
  tableHTML += '<caption style="font-size: 16px; font-weight: bold; margin-bottom: 10px;">❄️ Resultado da Prova de Refrigeração ❄️</caption>';
  tableHTML += '<tbody>';
  ...
  tableHTML += '<tr style="background-color: #006DCF; color: #fff;">'; // Cor de fundo e texto branco para o cabeçalho
  tableHTML += '<td style="border: 1px solid #ddd; padding: 8px; text-align: center;">📑 Questões</td>';
  tableHTML += '<td style="border: 1px solid #ddd; padding: 8px; text-align: center;">✍🏻 Respostas</td>';
  tableHTML += '<td style="border: 1px solid #ddd; padding: 8px; text-align: center;">🎯 Pontos</td>';
  tableHTML += '</tr>';
...
for (var i = 0; i < translatedValues.length; i++) {
  ...
  tableHTML += '<tr style="background-color: #006DCF; color: #fff;">'; // Cor de fundo e texto branco para a linha
  tableHTML += '<td colspan="2" style="border: 1px solid #ddd; padding: 8px; font-weight: bold; text-align: center;">🔢 Pontuação Total:</td>';
  tableHTML += '<td style="border: 1px solid #ddd; padding: 8px; text-align: center;">' + pontuacao + '</td>';
  tableHTML += '</tr>';
  ...
  var subject = '📝 Resultado da Prova Refrigeração Básico do ';
